...
import {
  ...
  ArrowLeft, RefreshCw, LayoutTemplate, FileImage, Wand2, Palette, Star, AlertTriangle, Package, Pencil,
} from "lucide-react";
...
const C = {
  ...
  safe: "rgba(232,80,70,0.18)", safeLine: "rgba(232,80,70,0.55)",
  zone: "rgba(238,94,44,0.16)", zoneLine: "rgba(238,94,44,0.6)",
};
...
function Primary({ children, onClick, disabled, full }) {
  return (<button onClick={onClick} disabled={disabled} className="inline-flex items-center justify-center" style={{ gap: 8, padding: "13px 24px", borderRadius: 12, border: "none", fontSize: 14, fontWeight: 600, color: "#fff", width: full ? "100%" : "auto", cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.4 : 1, background: `linear-gradient(180deg, ${C.flame}, ${C.flameDeep})`, boxShadow: disabled ? "none" : "0 10px 30px -12px rgba(238,94,44,0.65)" }}>{children}</button>);
}
...
function ScreenshotHint() {
  return (
    <div style={{ borderRadius: 14, background: C.panel, border: `1px solid ${C.line}`, padding: 12, marginBottom: 14 }}>
      ...
      <div style={{ color: C.mute, fontSize: 12, textAlign: "center", marginTop: 10, lineHeight: 1.4 }}>Take a screenshot of your product page or website and upload it here.</div>
    </div>
  );
}
...
function Wireframe({ d, theme }) {
  ...
  const Frame = ({ children }) => (
    <div style={{ position: "relative", width: "100%", maxWidth: 296, margin: "0 auto", aspectRatio: "9 / 16", borderRadius: 20, overflow: "hidden", background: theme.screen, border: `1px solid ${C.lineSoft}`, transition: "background .3s" }}>
      <div style={{ position: "absolute", left: 0, right: 0, top: 0, height: "11%", background: C.safe, borderBottom: `1px dashed ${C.safeLine}` }}><span style={{ position: "absolute", left: 8, top: 6, fontSize: 8, color: "#c9554c", fontWeight: 700 }}>TOP SAFE ZONE</span></div>
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: "11%", background: C.safe, borderTop: `1px dashed ${C.safeLine}` }}><span style={{ position: "absolute", left: 8, bottom: 6, fontSize: 8, color: "#c9554c", fontWeight: 700 }}>BOTTOM SAFE ZONE</span></div>
...
function FieldRow({ label, value, onChange, placeholder, onRegen, regenLoading }) {
...
function ToggleRow({ label, on, onToggle, value, onChange, placeholder, multiline, onRegen, regenLoading }) {
  return (<div style={{ marginBottom: 16, paddingBottom: 16, borderBottom: `1px solid ${C.line}` }}>
    ...
    {on && <div className="fade" style={{ marginTop: 9 }}><EditInput value={value} onChange={onChange} placeholder={placeholder} multiline={multiline} /></div>}
  </div>);
}
...
export default function CTADirector() {
  ...
  const prefs = useRef({ style: "aisuggest", inspo: "auto" });
  ...
  const regenerate = async (field) => {
    ...
    const ref = source === "website" ? "website screenshot" : "product image";
    const ask = {
      headline: "one fresh, punchy headline (max 7 words, benefit-led)",
      ...
      guarantee: "one fresh short guarantee line (max 8 words)",
      reviews: "one fresh short social-proof / reviews line (max 8 words); only use realistic framing and do NOT invent specific numbers that aren't visible",
    }[field];
    const text = `You are an elite DTC copywriter. Look closely at this ${ref} for the product "${draft.product || "(see image)"}". Write ${ask}. Keep it strictly on-brand and in the same tone, relevant to this exact product category and positioning, conversion-focused and realistic. Never invent unrealistic claims, prices, or unrelated marketing copy. Return ONLY the text itself — no quotes, labels or preamble.`;
    let out = "";
  ...
  const analyze = async () => {
    const ref = source === "website" ? "website screenshot" : "product image";
    const text = `You are an elite DTC performance-creative director. Analyze this ${ref} as the primary source of truth.
Return ONLY minified JSON, no markdown. Extract the EXACT values as they literally appear on screen: the real product name, the real main headline, the real CTA button label, the real price, the real review rating/count, the real guarantee text, and the real product benefits/trust points. Do NOT translate, paraphrase or invent. If you are not confident a value is actually present, return "" (or [] for benefits) — never use generic or placeholder marketing text.
{"analysis":{"product":"","brandColors":["#hex","#hex","#hex","#hex"],"headline":"","subheadline":"","price":"","ctaText":"","reviews":"","benefits":[],"guarantee":""}}`;
    let a = null;
    try { const t = await callClaude({ image: main, text, maxTokens: 1200 }); const j = parseJSON(t); a = j.analysis || {}; } catch (e) {}
    let fellBack = false;
    if (!a || !Object.keys(a).length) {
      ...
      a = { product: "", brandColors: colors, headline: "", subheadline: "", price: "", ctaText: "", reviews: "", benefits: [], guarantee: "" };
    }
    return { a, fellBack };
  };
  ...
  const generate = async () => {
    if (!main) return;
    setStep(2); setPhase("loading"); setNote(""); setDraft(null); setApplied(null); setAngle(0); setPct(0); setStatusMsg(LOAD_MSGS[0]);
    let fi = Math.floor(Math.random() * FACTS.length); setFact(FACTS[fi]);
    ...
    const fiv = setInterval(() => { fi = (fi + 1 + Math.floor(Math.random() * (FACTS.length - 1))) % FACTS.length; setFact(FACTS[fi]); }, 4000);
    const { a, fellBack } = await analyze();
    ...
    if (fellBack) setNote("Live AI analysis wasn't available — only brand colors were sampled. Fields are left empty so nothing is invented; toggle on what you need, fill it in, then Apply Changes.");
    const colors = (a.brandColors || []).filter(Boolean);
    ...
    const d0 = {
      product: a.product || "", ctaText: a.ctaText || "",
      headline: a.headline || "", subheadline: a.subheadline || "", price: a.price || "",
      reviews: a.reviews || "", guarantee: a.guarantee || "", benefits: a.benefits || [],
      brandColors: startColors,
      ctaPos: "Balanced", style: startStyle, inspo: prefs.current.inspo,
      flags: { headline: !!a.headline, subheadline: !!a.subheadline, price: !!a.price, reviews: !!a.reviews, guarantee: !!a.guarantee, benefits: !!(a.benefits && a.benefits.length) },
    };
  ...
  return (
    <div style={{ minHeight: "100vh", background: `radial-gradient(1200px 620px at 50% -12%, ${C.bgGrad}, ${C.bg})`, color: C.text, fontFamily: 'Inter, ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <header className="flex items-center" style={{ justifyContent: "space-between", padding: "18px 32px", borderBottom: `1px solid ${C.line}` }}><Logo /><StepNav step={step} /></header>

      <main style={{ padding: "52px 32px 40px" }}>
        {step === 1 && (
          <div className="fade mx-auto w-full" style={{ maxWidth: 640 }}>
            <div className="text-center" style={{ marginBottom: 32 }}>
              <h1 style={{ fontSize: 38, fontWeight: 800, letterSpacing: "-0.02em", margin: 0 }}>Drop assets. Get a world-class Flow prompt.</h1>
              <p style={{ color: C.mute, fontSize: 15, marginTop: 10 }}>The AI fills everything in. You tweak one or two things, then copy.</p>
            </div>
            ...
            <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 26 }}>
              {[{ k: "website", icon: <Layers size={20} />, t: "Website Screenshot", d: "Primary source of truth — product, colors, price, reviews and brand style detected automatically." },
                ...
                const on = source === o.k;
                return (<button key={o.k} onClick={() => { setSource(o.k); setMain(null); }} style={{ textAlign: "left", padding: 20, borderRadius: 16, cursor: "pointer", background: on ? C.flameSoft : C.panel, border: `1px solid ${on ? C.flame : C.line}`, boxShadow: on ? "0 12px 34px -16px rgba(238,94,44,0.6)" : "none", transition: "all .15s" }}>
            ...
            <div className="flex" style={{ justifyContent: "center", marginTop: 30 }}><Primary onClick={generate} disabled={!main}><Sparkles size={16} /> Analyze & build Flow prompt</Primary></div>
          </div>
        )}
        ...
        {step === 2 && phase === "done" && draft && (
          <div className="fade mx-auto w-full" style={{ maxWidth: 1060 }}>
            ...
            {note && <div style={{ background: C.flameSoft, border: `1px solid ${C.flameRing}`, color: "#f0a584", borderRadius: 12, padding: "11px 14px", fontSize: 13, marginBottom: 20 }}>{note}</div>}
            ...
            <div className="grid" style={{ gridTemplateColumns: "300px 1fr", gap: 20, alignItems: "start" }}>
              ...
              <div style={{ borderRadius: 16, background: C.panel, border: `1px solid ${C.line}`, padding: 20 }}>
                <div className="flex items-center" style={{ gap: 8, marginBottom: 18 }}><Sparkles size={15} color={C.flame} /><span style={{ fontSize: 13, fontWeight: 600 }}>What the AI detected</span><span style={{ color: C.faint, fontSize: 11, marginLeft: "auto" }}>edit freely · toggle off to exclude</span></div>
                <FieldRow label="PRODUCT" value={draft.product} onChange={sf("product")} placeholder="Detected product name" />
                <ToggleRow label="Headline" on={draft.flags.headline} onToggle={tf("headline")} value={draft.headline} onChange={sf("headline")} placeholder="Detected headline" onRegen={() => regenerate("headline")} regenLoading={regenField === "headline"} />
                <ToggleRow label="Subheadline" on={draft.flags.subheadline} onToggle={tf("subheadline")} value={draft.subheadline} onChange={sf("subheadline")} placeholder="Detected subheadline" onRegen={() => regenerate("subheadline")} regenLoading={regenField === "subheadline"} />
                <FieldRow label="CTA BUTTON TEXT" value={draft.ctaText} onChange={sf("ctaText")} placeholder="e.g. Shop Now" onRegen={() => regenerate("ctaText")} regenLoading={regenField === "ctaText"} />
                ...
                <ToggleRow label="Reviews" on={draft.flags.reviews} onToggle={tf("reviews")} value={draft.reviews} onChange={sf("reviews")} placeholder="e.g. 4.8 · 40K+ reviews" onRegen={() => regenerate("reviews")} regenLoading={regenField === "reviews"} />
                <ToggleRow label="Guarantee" on={draft.flags.guarantee} onToggle={tf("guarantee")} value={draft.guarantee} onChange={sf("guarantee")} placeholder="e.g. 30-day money-back guarantee" onRegen={() => regenerate("guarantee")} regenLoading={regenField === "guarantee"} />
                <ToggleRow label="Benefits" on={draft.flags.benefits} onToggle={tf("benefits")} value={(draft.benefits || []).join(", ")} onChange={setBenefits} placeholder="One per comma — e.g. Hydrates skin, Refines pores" multiline onRegen={() => regenerate("benefits")} regenLoading={regenField === "benefits"} />
                <div>
      ...
      <footer style={{ textAlign: "center", padding: "26px 0 40px", fontSize: 11, color: C.mute, opacity: 0.45, letterSpacing: "0.02em" }}>by Dima</footer>
    </div>
  );
}
