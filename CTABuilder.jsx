...
export default function CTADirector() {
  ...
  const regenerate = async (field) => {
    ...
    if (!ask) { setRegenField(null); return; }
    const text = `You are an elite DTC conversion copywriter. Look closely at this ${ref} for the product "${draft.product || "(see image)"}"${draft.category ? ` (category: ${draft.category})` : ""}. Write ${ask} — an OPTIMIZED marketing alternative, not a copy of the on-page text. Keep it strictly on-brand and in the same tone, relevant to this exact product category and positioning, premium and conversion-focused, and realistic. Never invent unrealistic claims, prices, or unrelated marketing copy. Return ONLY the text itself — no quotes, labels or preamble.`;
    let out = "";
...
{"detected":{"product":"","category":"","brandColors":["#hex","#hex","#hex","#hex"],"headline":"","subheadline":"","price":"","ctaText":"","reviews":"","benefits":[],"guarantee":""},"suggested":{"headline":"max 6 words, punchy benefit-led","subheadline":"max 12 words, supportive","ctaText":"max 3 words, action-led","benefits":["3 short benefits, each max 4 words"]}}`;
  ...
  const generate = async () => {
    ...
    const { det, sug, fellBack } = await analyze();
    ...
    if (fellBack) setNote("Live AI analysis wasn't available — only brand colors were sampled. Toggle on the fields you need, write your copy, then Apply Changes.");
  ...
  return (
    <div style={{ minHeight: "100vh", background: `radial-gradient(1200px 620px at 50% -12%, ${C.bgGrad}, ${C.bg})`, color: C.text, fontFamily: 'Inter, ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif' }}>
      <div aria-hidden style={{ position: "fixed", left: 0, right: 0, bottom: 0, height: "44vh", pointerEvents: "none", zIndex: 0, opacity: heroGlow * 0.45, background: `radial-gradient(120% 120% at 50% 150%, ${C.flame} 0%, rgba(238,94,44,0.30) 26%, rgba(238,94,44,0) 60%)`, transition: "opacity .5s cubic-bezier(.2,.7,.2,1)" }} />
      <style>{`
        ...
        @keyframes heroBloom {
          0%, 100% { text-shadow: 0 0 6px rgba(238,94,44,0.18), 0 0 16px rgba(238,94,44,0.10); }
          50% { text-shadow: 0 0 12px rgba(238,94,44,0.40), 0 0 30px rgba(238,94,44,0.22); }
        }
          ...
          <div className="fade mx-auto w-full" style={{ maxWidth: 1060 }}>
            ...
            <div className="grid" style={{ gridTemplateColumns: "300px 1fr", gap: 20, alignItems: "start" }}>
              ...
              <div style={{ borderRadius: 16, background: C.panel, border: `1px solid ${C.line}`, padding: 20 }}>
                ...
                <ToggleRow label="Guarantee" on={draft.flags.guarantee} onToggle={tf("guarantee")} value={draft.guarantee} onChange={sf("guarantee")} placeholder="e.g. 30-day money-back guarantee" />
